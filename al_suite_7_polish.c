...
/*   suite_8_power_process_helper("wilson5_0_record");exit(0); */
  ...
  int verbose=GLOBAL_verbose;
  char gs2[1024],command[4096];
  int nv=0,nt=0;
  int nowodor=0,nowconcentration=0,nowinstance=0,nowbicuculline=0;
  sprintf(gs2,"%srecord",GLOBAL_STRING_2);
  GLOBAL_verbose=1;
  suite_7_power_process_helper(gs2);
  if (SUITE_7_CLEANUP){ for (nt=0;nt<GLOBAL_POWER->indexing_ntype_length;nt++){ for (nv=0;nv<GLOBAL_POWER->indexing_nvar_length;nv++){ for (nowinstance=0;nowinstance<SUITE_NINSTANCES;nowinstance++){ for (nowconcentration=0;nowconcentration<SUITE_NCONCENTRATIONS;nowconcentration++){ for (nowodor=0;nowodor<SUITE_NODORS;nowodor++){ for (nowbicuculline=0;nowbicuculline<SUITE_NBICUCULLINES;nowbicuculline++){ sprintf(command,"""rm"" ./dir_temp_suite_7_%s/power_deposit_type_%s_var_%s_instance_%d_concentration_%d_odor_%d_bicuculline_%d_%s;",gs2,GLOBAL_TYPENAMES[GLOBAL_POWER->indexing_ntype_checkout[nt]],GLOBAL_VARNAMES[GLOBAL_POWER->indexing_nvar_checkout[nv]],nowinstance,nowconcentration,nowodor,nowbicuculline,gs2); if (verbose){ printf("%s\n",command);} system(command);}}}}}}}
  sprintf(command,"cd dir_temp_suite_7_%s; nice ./ptree_test < Lra_t_pn_b_0_%s.in;",gs2,gs2);system(command);  
  exit(0);
  ...
  if (SUPERGLOBAL_DRAW_FLAG){
    ...
    GLUTWINDOWNUMBER = glutCreateWindow("al_suite_7_polish"); /* open window */
